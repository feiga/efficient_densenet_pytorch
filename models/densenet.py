...
def _atten_function_factory(attention):
    def bn_function(*inputs):
        ...
        return atten_features #bottleneck_output
...
class _DenseLayer(nn.Module):
    def __init__(self, num_input_features, bn_size, drop_rate, efficient=False):
    ...
    def forward(self, *prev_features):
        bn_function = _atten_function_factory(self.depth_attention)
        if self.efficient and any(prev_feature.requires_grad for prev_feature in prev_features):
            atten_features = cp.checkpoint(bn_function, *prev_features)
        else:
            atten_features = bn_function(*prev_features)
        new_features = self.conv1(self.relu1(self.norm1(atten_features)))
...
class _DenseBlock(nn.Module):
    def __init__(self, num_layers, num_input_features, bn_size, drop_rate, efficient=False):
    ...
    def forward(self, init_features):
        ...
        return features[-1] #torch.cat(features, 1)
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                trans = _Transition(num_input_features=num_features,
                                    num_output_features= num_features * 2) #int(num_features * compression))
